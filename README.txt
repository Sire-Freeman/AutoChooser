...
>Installation<
Place the AutoChooser()().exe and The Pathway.exe into the same folder as your Sound Libarary Folder.
...
The program works off the information inside the two brackets. ONLY requiring the user to keep the name 'auto chooser' and two sets of brackects to work.

Example:
Demo AutoChooser version ('Sound Folder Name')('Translated directory of the folder location') aisjdianfaef.exe

>The Pathway.exe<
This acts as a easier user interface to create,adapt and organise through three different options:

1. AutoChooser Generator - Allows the user to create autochoosers based on the folders within the same directory. The user can select folders to generate the AutoChoosers for.

2. Automatic - You can duplicate and adapt the Auto Chooser.exe yourself however instead translating the directory yourself, you can set the directory for all the Autochoosers in the same folder.
...
Can only execute one folder and not sub folders within the folder.
